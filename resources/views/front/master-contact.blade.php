...
<body class="site-body style-v1">
	...
	<script>
		document.addEventListener('DOMContentLoaded', function () {
			const form = document.getElementById('quote-request');
			const resultDiv = document.querySelector('.form-results');
			...
			form.addEventListener('submit', function (e) {
				...
				// Add the reCAPTCHA response if needed
				const recaptchaResponse = grecaptcha.getResponse();
				formData.append('g-recaptcha-response', recaptchaResponse);

				fetch("{{ route('membership.post') }}", {
					method: "POST",
					headers: {
				...
				.then(data => {
					resultDiv.innerHTML = `<div class="alert alert-success">${data.message || 'Form submitted successfully.'}</div>`;
					form.reset();
					grecaptcha.reset();
				})
				.catch(error => {
